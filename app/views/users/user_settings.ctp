...
<form action="?" method="post" >
    <div class="detailed_inputs">
        <div>
            <h3><?php __('Email Address'); ?> <span class="small"><?php __('An email address we can contact you at.'); ?></span></h3>
            <input type="text" name="data[User][email]" value="<?= $user_info['User']['email']; ?>"/>
        </div>
        <div>
            <h3><?php __('Age'); ?> <span class="small"><?php __('An optional age to show.'); ?></span></h3>
            <input type="text" maxlength="3" name="data[User][age]" value="<?= $user_info['User']['age']; ?>"/>
        </div>
        <div>
            <h3><?php __('Location'); ?> <span class="small"><?php __('Where are you located?'); ?></span></h3>
            <input type="text" name="data[User][location]" value="<?= $user_info['User']['location']; ?>"/>
        </div>
        <div>
            <h3><?php __('Website'); ?> <span class="small"><?php __('Have a website or social profile?'); ?></span></h3>
            <input type="text" name="data[User][website]" value="<?= $user_info['User']['website']; ?>"/>
        </div>
        <div>
            <h3><?php __('Summary'); ?> <span class="small"><?php __('Tell us a little about yourself.'); ?></span></h3>
            <textarea name="data[User][info]"><?= $user_info['User']['info']; ?></textarea>
        </div>
        <div>
            <h3><?php __('Password Change'); ?> <span class="small"><?php __('Set a new password. Leave blank if you do not wish to change your password.'); ?></span></h3>
            <input type="password" name="data[User][new_password]"/>
        </div>
        <div id="old_password">
            <h3><?php __('Old Password'); ?> <span class="small"><?php __('If setting a new password please enter your current password.'); ?></span></h3>
            <input type="password" name="data[User][old_password]"/>
        </div>
        <div class="submit">
            <input type="submit" value="<?php __('Update Your Account'); ?>"/>
        </div>
    </div>
</form>